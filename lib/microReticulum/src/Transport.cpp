				...
				if (packet.transport_id()) {
					...
					if (Reticulum::transport_enabled() && _announce_table.count(packet.destination_hash()) > 0) {
						...
						if (!announce_erased && (packet.hops() - 1) == (announce_entry._hops + 1) && announce_entry._retries > 0) {
							double now = OS::time();
							if (now < announce_entry._retransmit_timeout) {
...
*/
						...
						// Cap random_blobs to limit filesystem storage usage
						while (random_blobs.size() > 5) {
...
/*static*/ Transport::DestinationEntry& Transport::get_path(const Bytes& destination_hash) {
	...
	// Note: announce_packet validation removed here — relay only needs
	// receiving_interface, received_from, and hops. The cached announce
	// packet may be pruned from flash to save storage; its absence should
	// not prevent packet relay. It is validated in path_request() where
...
/*static*/ void Transport::path_request(const Bytes& destination_hash, bool is_from_local_client, const Interface& attached_interface, const Bytes& requestor_transport_id /*= {}*/, const Bytes& tag /*= {}*/) {
    ...
    //p elif (RNS.Reticulum.transport_enabled() or is_from_local_client) and (destination_hash in Transport.destination_table):
		...
		if (!announce_packet) {
			DEBUGF("Cannot answer path request for %s, cached announce packet missing from storage", destination_hash.toHex().c_str());
		}
		else {
